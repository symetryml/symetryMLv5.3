...
to answer.
TODO : add a section on sequence models?
...
ones the model was build with, will yield a higher anomaly score than those which are more inline with what
the model has seen before.
TODO : add a section on anomaly models
...
*float* or *double*. Use of doubles is generally preferred as they offer higher levels of precision, but if
memory issues are encountered, switching to a *float* backed model should reduce the memory footprint.
TODO: add mgpuf and mgpud difference
...
As with other Online Models, RF parameters must be specified at the beginning of project creation.
See section <<rf-projects>> for additional details and the specifics of the parameters required.

TODO: update for random forest regression and anomaly
...
histograms allow a project to create HBA Models (TODO Create link to HBA). While the information provided by the histograms
can often times be invaluable, they do slow down the speed of project updates. Hence, they should be used sparingly in production environments
that require fast learning.

TODO: update for histogram
...
TODO: update for regression and anomaly
...
The required fields when selecting this option are : *RF Type*, *Target Column*, and *Number of Classes*.
The inputs of the model can also be specified. If left unselected all of the inputs will be used.
TODO: update for regression and anomaly

image::media/web/p_advanced_rf1.png[title='Advanced Options - RF', width=50%]
{nbsp} +
